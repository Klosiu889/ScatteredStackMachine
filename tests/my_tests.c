...
// Tę funkcję woła rdzeń.
void print_register(uint64_t n, char v, int u) {
    printf("Core %lu printed value %c and %d\n", n, v, u);
}
...
int main() {
    ...
    static const test_t tests[N] = {
            {.computation = "27*1S2ED+E1-+75+-BC", .name = "Example", .result = 112},
            {.computation = "47*0S2ED+E1-+75+-BC", .name = "Example 2", .result = 56},
